...
class G1Region extends Box {
	...
	garbageContents = fitChild(new Box(0, ceil(maxHeight * (1 - fillFactor)),
					   width, maxHeight * fillFactor, occupiedColor, garbageAlpha * contentsAlphaCoefficient));
    }
    ...
    void setGarbageAlpha(alpha) {
	garbageContents.alpha = alpha * contentsAlphaCoefficient;
	garbageAlpha = alpha;
    }
	...
	fill(#000000, garbageAlpha);
	text("X", x, y, width, height);
    }
}
...
ArrayList forthFragmentAnimations = new ArrayList();
...
void setup() {  
    ...
    g1Heap.updateRegionLabels("E", "", "", "", "", "O", "O", "", "", "", "", "O", "",
			"", "O", "E", "S", "O", "", "O", "O", "O", "E", "", "", "O",
			"O", "O", "O", "O", "", "O", "", "", "O", "", "O", "O", "");
...
void addAnimations() {
    int waitFillDuration = 0;
    int fillDuration = 2000;
    var changeOccupancyFunction = function(occupancy) { this.setOccupancy(occupancy); };
    var isNotFillingCompletely = function(region) { return region === g1Heap.getRegion(2,1) || region === g1Heap.getRegion(3,1) || region === g1Heap.getRegion(11,0); };
	...
	if (currentRegion.label !== "") {
	    firstFragmentAnimations.add(addInactiveAnimation(currentRegion, changeOccupancyFunction,
							     currentRegion.getOccupancy(), isNotFillingCompletely(currentRegion) ? 0.9 : 1,
							     fillDuration, waitFillDuration));
	}
    }

    int waitMinorCollectionsDuration = isInPresentation() ? 0 : waitFillDuration + fillDuration + 1000;
    int minorCollectionsDuration = 2000;
    ArrayList vanishingRegions = new ArrayList();
    vanishingRegions.add(g1Heap.getRegion(0,0));
    vanishingRegions.add(g1Heap.getRegion(2,1));
    vanishingRegions.add(g1Heap.getRegion(3,1));
    vanishingRegions.add(g1Heap.getRegion(9,1));

    var changeInteriorAlphaFunction = function(alpha) { this.setContentsAlpha(alpha); };
    for (int i = 0; i < vanishingRegions.size(); i++) {
	secondFragmentAnimations.add(addInactiveAnimation(vanishingRegions.get(i), changeInteriorAlphaFunction,
							  vanishingRegions.get(i).contentsAlpha, 0,
							  minorCollectionsDuration, waitMinorCollectionsDuration));
    }

    ArrayList newEdenRegions = new ArrayList();
    newEdenRegions.add(g1Heap.getRegion(2,0));
    newEdenRegions.add(g1Heap.getRegion(6,2));
    G1Region newSurvivorRegion = g1Heap.getRegion(4,0);
    var setupNewRegionFunction = function(label) {
	return function() {
	    this.setContentsAlpha(0); this.label = label; this.setOccupancy(0.3 + 0.7*Math.random());
	};
    };
    for (int i = 0; i < newEdenRegions.size(); i++) {
	secondFragmentAnimations.add(addInactiveAnimation(newEdenRegions.get(i), setupNewRegionFunction("E"),
							  0, 1, 0, waitMinorCollectionsDuration));
	secondFragmentAnimations.add(addInactiveAnimation(newEdenRegions.get(i), changeInteriorAlphaFunction,
							  0, 255,
							  minorCollectionsDuration, waitMinorCollectionsDuration));
    }
    secondFragmentAnimations.add(addInactiveAnimation(newSurvivorRegion, setupNewRegionFunction("S"),
						      0, 1, 0, waitMinorCollectionsDuration));
    secondFragmentAnimations.add(addInactiveAnimation(newSurvivorRegion, changeInteriorAlphaFunction,
						      0, 255,
						      minorCollectionsDuration, waitMinorCollectionsDuration));

    int waitMarkDuration = isInPresentation() ? 0 : waitMinorCollectionsDuration + minorCollectionsDuration + 1000;
    int markDuration = 2000;
    ArrayList markedRegions = new ArrayList();
    markedRegions.add(g1Heap.getRegion(6,0));
    markedRegions.add(g1Heap.getRegion(4,1));
    markedRegions.add(g1Heap.getRegion(6,1));
    markedRegions.add(g1Heap.getRegion(12,1));
    markedRegions.add(g1Heap.getRegion(1,2));
    markedRegions.add(g1Heap.getRegion(8,2));
    for (int i = 0; i < markedRegions.size(); i++) {
	G1Region currentRegion = markedRegions.get(i);
	thirdFragmentAnimations.add(addInactiveAnimation(currentRegion, function (value) {this.setGarbageAlpha(value);},
							 0, 255,
							 markDuration, waitMarkDuration));
	
	thirdFragmentAnimations.add(addInactiveAnimation(markedRegions.get(i), changeInteriorAlphaFunction,
							 255, 0,
							 markDuration / 2, waitMarkDuration));
    
    }

    int waitAddOldDuration = isInPresentation() ? 0 : waitMarkDuration + markDuration + 1000;
    int addOldDuration = 2000;
    G1Region newOldRegion = g1Heap.getRegion(0, 1);
    forthFragmentAnimations.add(addInactiveAnimation(newOldRegion, setupNewRegionFunction("O"),
						      0, 1, 0, waitAddOldDuration));
    forthFragmentAnimations.add(addInactiveAnimation(newOldRegion, changeInteriorAlphaFunction,
						      0, 255,
						      addOldDuration, waitAddOldDuration));
